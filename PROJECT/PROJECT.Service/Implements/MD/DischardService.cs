...
{
    public class DischardService : GenericService<T_MD_DISCHARD, T_MD_DISCHARD_Dto>, IDischardService
    {
        private readonly AppDbContext _context;
        public DischardService(AppDbContext context, IMapper mapper) : base(context, mapper)
        {
            _context = context;
        }
        public async Task<DischardFilter> Search(DischardFilter page)
        {
            var query = _context.T_MD_DISCHARD.AsQueryable();
        ...
        public async Task<TranferObject> Update(T_MD_DISCHARD request)
        {
            try
            {
                _context.T_MD_DISCHARD.Update(request);
