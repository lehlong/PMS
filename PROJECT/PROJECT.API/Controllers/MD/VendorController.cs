...
namespace PROJECT.API.Controllers.MD
{
    ...
    {
        public readonly IVendorService _service;
        ...
        [Route("GetList")]
        public async Task<IActionResult> GetList([FromQuery] VendorFilter page)
        {
        ...
        //[Right(ClaimTypes.Role, "R1, R192")]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] T_MD_VENDOR Vendor)
        {
            var tranferObject = new TranferObject();
            var pagination = await _service.Update(Vendor);
            if (_service.Status)
            {
                tranferObject.Data = pagination;
            }
